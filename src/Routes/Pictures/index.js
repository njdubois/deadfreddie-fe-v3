...
function Pictures() {
  ...
  const [showLatestPhotos, setShowLatestPhotos] = useState(false);
  const [showPhotoArchive, setShowPhotoArchive] = useState(false);
  const [showPastFlyers, setShowPastFlyers] = useState(false);

  const ToggleGallery = (gallery) => {
    setShowLatestPhotos(false);
    setShowPhotoArchive(false);
    setShowPastFlyers(false);

    switch (gallery) {
      case "LatestPhotos":
        setShowLatestPhotos(true);
        break;
      case "PhotoArchive":
        setShowPhotoArchive(true);
        break;
      case "PastFlyers":
        setShowPastFlyers(true);
        break;
    }
  };

  const RenderLatestPhotos = () => {
    return (
      <StyledRow>
        <StyledCol>
  ...
  const RenderPhotoArchive = () => {
    return (
      <StyledRow>
        <StyledCol>
          {PHOTO_ARCHIVE.map((photoCollection, key) => {
            return (
              <StyledRow key={key}>
                <StyledCol>
                  <SetDescription>
                    {photoCollection.year}
                  </SetDescription>
                  {photoCollection.sets.map((set, key) => (
                    <StyledRow>
                      <StyledCol>
                        <SetDescription>
                          {set.description}
  ...
  return (
    <>
      <PhotoMenu>
        <PhotoMenuItem selected={showLatestPhotos} onClick={() => ToggleGallery('LatestPhotos')}>
          Latest Photos
        </PhotoMenuItem>
        <PhotoMenuItem selected={showPhotoArchive} onClick={() => ToggleGallery('PhotoArchive')}>
          Photo Archive
        </PhotoMenuItem>
        <PhotoMenuItem selected={showPastFlyers} onClick={() => ToggleGallery('PastFlyers')}>
          Past Flyers
        </PhotoMenuItem>
      </PhotoMenu>
